// IMPORTANT: This file should be located in the `api` directory at the root of your project.
// Vercel will automatically turn this into a serverless function.

import { GoogleGenAI, GenerateContentResponse } from "@google/genai";

// This is a serverless function, so the API key is a server-side environment variable.
...
if (apiKey) {
    ...
    console.warn("API_KEY environment variable not found on server. AI functionality will be disabled.");
}
...
## CONTEXTO
El usuario te proporcionará una idea o un borrador. Tu tarea es expandir y estructurar esa idea, no simplemente parafrasearla. Debes añadir capas de detalle, contexto y directivas que el usuario no consideró, pero que son crucales para un buen resultado.
...
// Using 'any' for request and response types for compatibility in different serverless environments.
export default async function handler(request: any, response: any) {
    ...
    if (!ai) {
        return response.status(500).json({ error: "El cliente de IA no está configurado en el servidor." });
    }
    ...
    try {
        ...
        if (type === 'generate') {
            const { idea, fileContext } = payload;
             if (!idea.trim() && !fileContext) {
                return response.status(400).json({ error: "Por favor, proporciona una idea o un archivo de contexto para el prompt." });
            }
            ...
            const { idea, fileContext, currentPrompt, feedback } = payload;
            if (!currentPrompt || !feedback) {
                return response.status(400).json({ error: "No se puede refinar sin el prompt actual y el feedback." });
            }
            ...
            geminiResponse = await ai.models.generateContent({
                model: 'gemini-2.5-flash',
                contents: userContent,
                config: { systemInstruction: REFINE_SYSTEM_INSTRUCTION },
            });
        } else {
        ...
        if (text) {
            ...
            return response.status(500).json({ error: "La IA no generó una respuesta de texto válida." });
        }

    } catch (error) {
        console.error("Error calling Gemini API from serverless function:", error);
        const errorMessage = error instanceof Error ? error.message : "Error desconocido.";
        return response.status(500).json({ error: `No se pudo comunicar con el asistente de IA. Detalles: ${errorMessage}` });
    }
}
